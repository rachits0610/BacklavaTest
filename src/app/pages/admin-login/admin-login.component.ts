...
//     const email = this.forgotForm.value.forgotEmail!;
//     const payload = { email: email, otp: this.otpValue };
...
/** Rejects any string that contains emoji or other non-BMP Unicode characters */
function noEmojiValidator(): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    const val: string = control.value ?? '';
    // Matches emoji, pictographs, symbols, regional indicators, etc.
    const emojiRegex =
...
/** Rejects strings that contain whitespace characters */
...
/** Trims leading/trailing whitespace and runs the standard email regex */
...
export class AdminLoginComponent implements OnDestroy {
  ...
  /** Call this on (input) for any field that should not allow emoji. */
  stripEmoji(event: Event): void {
    ...
    if (cleaned !== input.value) {
      ...
      // Tell Angular reactive forms the value changed
  ...
  onSubmit(): void {
    ...
    this.authService.login(email!, password!).subscribe({
      ...
      error: (err: any) => {
        this.loading = false;
        this.error =
          err.error?.meta?.status_message || 'Login failed. Please try again.';
      },
    });
  }
